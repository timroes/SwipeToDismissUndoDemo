...
/**
 * A simple ListActivity (extending {@link SherlockListActivity} for backwards 
...
public class MainActivity extends SherlockListActivity {
	...
	/**
	 * The adapter used to store the the list data. In this case an {@link ArrayAdapter} 
	 * is used. This will be replaced by your adapter.
	 */
    private ArrayAdapter<String> mAdapter;
	
	/**
	 * The {@link SwipeDismissList} you generate during your {@link #onCreate(android.os.Bundle)}
	 * method.
	 */
    private SwipeDismissList mSwipeList;

	/**
	 * The key of the {@link Bundle} extra we store the mode of the list the user
	 * selected.
	 */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSwipeList = new SwipeDismissList(
            ...
            new SwipeDismissList.OnDismissCallback() {
				 ...
				 * 
				 * @param listView The {@link ListView} that the item was deleted from.
                ...
                public SwipeDismissList.Undoable onDismiss(ListView listView, final int position) {
                    ...
                    return new SwipeDismissList.Undoable() {
						...
						/**
						 * Optional method. If you implement this method, the returned
						 * String will be presented in the undo view to the user.
						 */
                        @Override
						...
						/**
						 * Will be called when the user hits undo. You want to reinsert
						 * the item to the adapter again. The library will always call
						 * undo in the reverse order the item has been deleted. So you can
						 * insert the item at the position it was deleted from, unless
						 * you have modified the list (added or removed items) somewhere
						 * else in your activity. If you do so, you might want to call
						 * {@link SwipeDismissList#discardUndo()}, so the user cannot
						 * undo the action anymore. If you still want the user to be
						 * able to undo the deletion (after you modified the list somewhere else)
						 * you will need to calculate the new position of this item yourself.
						 */
						@Override
                        public void undo() {
						...
						/**
						 * Will be called, when the user doesn't have the possibility to
						 * undo the action anymore. This can either happen, because the
						 * undo timed out or {@link SwipeDismissList#discardUndo()} was
						 * called. If you have stored your objects somewhere persistent
						 * (e.g. a database) you might want to use this method to 
						 * delete the object from this persistent storage.
						 */
						@Override
						public void discard() {
	 ...
	 * {@link SwipeDismissList.Undoable#discard()} is not guaranteed to be called
	 * for every item. If you use the discard method to delete items from database,
	 * not calling discardUndo() will result in bugs.
	 */
	@Override
